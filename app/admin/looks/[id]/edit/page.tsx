...
export default function EditCompleteLookPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          bundlePrice: look.bundlePrice ? (look.bundlePrice / 100).toFixed(2) : '',
          bundleDiscount: look.bundleDiscount ? String(look.bundleDiscount) : '',
          featured: look.featured || false,
          isActive: look.isActive !== undefined ? look.isActive : true,
          ageRange: look.ageRange || '',
          tags: look.tags || [],
        });
        const productIds = look.products?.map((p: any) => p.productId || p.product?.id) || [];
        setSelectedProducts(productIds);
        const requiredIds = look.products?.filter((p: any) => p.isRequired !== false).map((p: any) => p.productId || p.product?.id) || [];
        setRequiredProducts(new Set(requiredIds));
      } else {
        showToast({
          type: "error",
          title: "Load Failed",
      ...
      showToast({
        type: "error",
        title: "Load Failed",
        message: "An error occurred while loading the complete look",
      });
    } finally {
      setLoading(false);
    }
      ...
      if (response.ok) {
        ...
        if (data.errors && typeof data.errors === 'object') {
          ...
          showToast({
            type: "error",
            title: "Update Failed",
            message: data.error || data.message || 'Failed to update complete look',
          });
        }
      }
    } catch (error) {
      if (process.env.NODE_ENV === 'development') {
        console.error('Error updating complete look:', error);
      }
      showToast({
        type: "error",
        title: "Update Failed",
        message: 'An error occurred while updating the complete look',
      });
    } finally {
      setSaving(false);
    }
    ...
    return (
      <div className="p-6">
        <div className="flex items-center justify-center h-64">
          <div className="text-charcoal-600">Loading complete look...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      ...
      <h1 className="text-3xl font-bold mb-8">Edit Complete Look</h1>
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl space-y-6">
        ...
        <div className="flex gap-4">
          ...
          <Button type="submit" disabled={saving || selectedProducts.length < 2}>
