-- ============================================
-- Migration: Update AdminRole Enum
...
-- Step 5: Drop old enum (only if it exists and is different from the new one)
DO $$ 
BEGIN
    -- Check if old enum exists (and is not the new one we just created)
    IF EXISTS (
        SELECT 1 
        FROM pg_type 
        WHERE typname = 'AdminRole' 
        AND oid NOT IN (SELECT oid FROM pg_type WHERE typname = 'AdminRole_new')
    ) THEN
        DROP TYPE "AdminRole";
    END IF;
EXCEPTION
    WHEN OTHERS THEN 
        ...
        RAISE NOTICE 'Could not drop old AdminRole enum: %', SQLERRM;
END $$;

-- Step 6: Rename new enum to original name (only if it doesn't already exist)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'AdminRole_new') THEN
        -- Check if AdminRole already exists (might have been renamed already)
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'AdminRole') THEN
            ALTER TYPE "AdminRole_new" RENAME TO "AdminRole";
        ELSE
            -- AdminRole already exists, just drop the new one
            DROP TYPE "AdminRole_new";
        END IF;
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Could not rename enum: %', SQLERRM;
END $$;
